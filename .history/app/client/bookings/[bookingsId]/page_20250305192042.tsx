...
export default function BookingDetailPage() {
  ...
  const { bookingsId } = useParams() // Changed from bookingId to bookingsId to match folder structure

  const loadBooking = () => {
    // Try sessionStorage first (set by Bookings list page)
    const sessionBooking = sessionStorage.getItem("selectedBooking")
    if (sessionBooking) {
      const parsedBooking = JSON.parse(sessionBooking)
      if (parsedBooking.id === bookingsId) {
    ...
    const foundBooking = savedBookings.find((b: Booking) => b.id === bookingsId)
    setBooking(foundBooking || null)
    console.log("Loaded booking from localStorage:", foundBooking || "Not found")
  }
      ...
      <ProtectedRoute allowedRoles={["client"]}>
        <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
          <div className="container mx-auto px-4 py-12 max-w-6xl text-center">
            ...
            <p className="text-muted-foreground mb-6">The booking with ID #{bookingsId} could not be found.</p>
