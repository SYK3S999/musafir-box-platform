...
function BookingPageContent() {
  ...
  const handleBookingConfirmation = () => {
    ...
    if (selectedOfferData) {
      ...
      // Save to localStorage with explicit error checking
      try {
        const existingBookings = JSON.parse(localStorage.getItem("bookings") || "[]")
        console.log("Current bookings in localStorage:", existingBookings)
        const updatedBookings = [...existingBookings, bookingData]
        localStorage.setItem("bookings", JSON.stringify(updatedBookings))
        console.log("Successfully saved booking to localStorage:", updatedBookings)

        // Verify save
        const verifiedBookings = JSON.parse(localStorage.getItem("bookings") || "[]")
        console.log("Verified bookings after save:", verifiedBookings)
      } catch (error) {
        console.error("Error saving booking to localStorage:", error)
      }

      console.log("Redirecting to /client/bookings in 2 seconds")
      setTimeout(() => {
        router.push(`/client/bookings`)
      }, 2000)
    } else {
      console.error("No offer found for ID:", selectedOffer)
    }
  }
