...
const getNavItems = (role: string | undefined) => {
  ...
  const clientItems = [
    { name: "Travel Plans", path: "/client/bookings", icon: Calendar },
      ...
      <div className="container flex h-20 items-center px-4 sm:px-6 lg:px-8">
        ...
        <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
          ...
          <SheetContent side="left" className="w-[300px] sm:w-[400px]">
            <MobileNav navItems={navItems} setIsMobileMenuOpen={setIsMobileMenuOpen} />
          </SheetContent>
        </Sheet>
...
const MobileNav = ({ navItems, setIsMobileMenuOpen }: { 
      ...
      <nav className="px-3">
        {navItems.map((item) => (
          <Link key={item.path} href={item.path} onClick={() => setIsMobileMenuOpen(false)}>
            <motion.div
              className="px-4 py-3 flex items-center space-x-2 text-lg rounded-lg transition-all duration-300 hover:bg-primary/10"
